...
TEST_CASE("SSE basic guarantees", "[simd_t][x86][sse]") {
    REQUIRE(F::W == 4);
    REQUIRE(U::W == 4);
    REQUIRE(S::W == 4);
    REQUIRE((std::is_same<F::mm_t, __m128>::value));
    REQUIRE((std::is_same<U::mm_t, __m128>::value));
    REQUIRE((std::is_same<S::mm_t, __m128>::value));
...
TEST_CASE("SSE explicit construction", "[simd_t][x86][sse]") {
    ...
    SECTION("from mm_t") {
        F tf(_mm_load_ps(bufAF.data()));
        U tu(_mm_load_ps((float*)bufAU.data()));
        S ts(_mm_load_ps((float*)bufAS.data()));
        tor(tf, tu, ts);
        REQUIRE(rf == bufAF);
        REQUIRE(ru == bufAU);
        REQUIRE(rs == bufAS);
...
TEST_CASE("SSE implicit construction", "[simd_t][x86][sse]") {
    alignas(F)F::array_e rf = bufZF;
    alignas(U)U::array_e ru = bufZU;
    alignas(S)S::array_e rs = bufZS;
    ...
    SECTION("from mm_t") {
        implicit_test(
            _mm_load_ps(bufAF.data()),
            _mm_load_ps((float*)bufAU.data()),
            _mm_load_ps((float*)bufAS.data()));
        REQUIRE(rf == bufAF);
        REQUIRE(ru == bufAU);
        REQUIRE(rs == bufAS);
    }
...
TEST_CASE("SSE assignment", "[simd_t][x86][sse]") {
    alignas(F)F::array_e rf = bufZF;
    alignas(U)U::array_e ru = bufZU;
    alignas(S)S::array_e rs = bufZS;
    F tf;
    ...
    SECTION("from mm_t") {
        tf = _mm_load_ps(bufAF.data());
        tu = _mm_load_ps((float*)bufAU.data());
        ts = _mm_load_ps((float*)bufAS.data());
        tor();
        REQUIRE(rf == bufAF);
        REQUIRE(ru == bufAU);
        REQUIRE(rs == bufAS);
...
TEST_CASE("SSE arithmetic", "[simd_t][x86][sse]") {
...
TEST_CASE("SSE comparison", "[simd_t][x86][sse]") {
...
TEST_CASE("SSE bitwise", "[simd_t][x86][sse]") {
    alignas(U)U::array_e r;
    alignas(U)U::array_e e;
...
TEST_CASE("SSE horizontal operations", "[simd_t][x86][sse]") {
    F a = simd::aligned(bufAF.data());
    F::e_t v, v0;
    uint32_t idx;
...
TEST_CASE("SSE conditional", "[simd_t][x86][sse]") {

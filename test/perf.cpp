...
TEST_CASE("bench_negate", "[!hide][perf]") {
    std::cout << hline << "Benchmark: Negate\n";
    volatile int sink = 0;
    const int nIter = 200 * 1024 * 1024;

    auto sseVariant1 = [nIter, &sink]() {
        __m128 res = _mm_setzero_ps();
        for (int i = 0; i < nIter; ++i) {
            __m128 temp = _mm_set_ps1(0x80000000);
            res = _mm_xor_ps(temp, res);
            sink = _mm_movemask_ps(res);
        }
    };

    auto sseVariant2 = [nIter, &sink]() {
        __m128 res = _mm_setzero_ps();
        for (int i = 0; i < nIter; ++i) {
            __m128 temp = _mm_setzero_ps();
            res = _mm_sub_ps(temp, res);
            sink = _mm_movemask_ps(res);
        }
    };

    auto sseSimdee = [nIter, &sink]() {
        sd::ssef res = sd::zero();
        for (int i = 0; i < nIter; ++i) {
            res = -res;
            sink = _mm_movemask_ps(res.data());
        }
    };

    auto avxVariant1 = [nIter, &sink]() {
        __m256 res = _mm256_setzero_ps();
        for (int i = 0; i < nIter; ++i) {
            auto magic = 0x80000000U;
            __m256 temp = _mm256_broadcast_ss((float*)&magic);
            res = _mm256_xor_ps(temp, res);
            sink = _mm256_movemask_ps(res);
        }
    };

    auto avxVariant2 = [nIter, &sink]() {
        __m256 res = _mm256_setzero_ps();
        for (int i = 0; i < nIter; ++i) {
            __m256 temp = _mm256_setzero_ps();
            res = _mm256_sub_ps(temp, res);
            sink = _mm256_movemask_ps(res);
        }
    };

    auto avxSimdee = [nIter, &sink]() {
        sd::avxf res = sd::zero();
        for (int i = 0; i < nIter; ++i) {
            res = -res;
            sink = _mm256_movemask_ps(res.data());
        }
    };

    std::cout << "SSE variant 1: " << benchmark_ms(sseVariant1) << " ms\n";
    std::cout << "SSE variant 2: " << benchmark_ms(sseVariant2) << " ms\n";
    std::cout << "SSE Simdee:    " << benchmark_ms(sseSimdee) << " ms\n";
    std::cout << "AVX variant 1: " << benchmark_ms(avxVariant1) << " ms\n";
    std::cout << "AVX variant 2: " << benchmark_ms(avxVariant2) << " ms\n";
    std::cout << "AVX Simdee:    " << benchmark_ms(avxSimdee) << " ms\n";
}

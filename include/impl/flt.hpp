...
namespace simd {
    ...
    struct flt : simd_base<float, float, uint32_t, flt> {
        ...
        SIMDIFY_FORCE_INLINE flt(const flt& r) : simd_base(r.mm) {}
        SIMDIFY_FORCE_INLINE explicit flt(zero_t) : simd_base(0) {}
        SIMDIFY_FORCE_INLINE explicit flt(mask_t r) : flt(r.f) {}

        SIMDIFY_FORCE_INLINE fp_t front() const { return mm; }
        SIMDIFY_FORCE_INLINE fp_t back() const { return mm; }
    };

    SIMDIFY_FORCE_INLINE const flt operator&(const flt& l, const flt& r) { return flt::mask_t::andf(l.mm, r.mm); }
    SIMDIFY_FORCE_INLINE const flt operator|(const flt& l, const flt& r) { return flt::mask_t::orf(l.mm, r.mm); }
    SIMDIFY_FORCE_INLINE const flt operator^(const flt& l, const flt& r) { return flt::mask_t::xorf(l.mm, r.mm); }
    SIMDIFY_FORCE_INLINE const flt operator~(const flt& l) { return flt::mask_t::notf(l.mm); }
    ...
    SIMDIFY_FORCE_INLINE const flt operator*(const flt& l, const flt& r) { return l.mm * r.mm; }
    SIMDIFY_FORCE_INLINE const flt operator/(const flt& l, const flt& r) { return l.mm / r.mm; }
    SIMDIFY_FORCE_INLINE const flt andnot(const flt& l, const flt& r) { return flt::mask_t::andnotf(l.mm, r.mm); }
    ...
    struct horizontal_impl<flt> : horizontal_impl_base<flt>{
        ...
        static SIMDIFY_FORCE_INLINE find_result find(const flt& in) { return in.i[0] != 0; }
        static SIMDIFY_FORCE_INLINE bool any(const flt& in) { return in.i[0] != 0; }
        static SIMDIFY_FORCE_INLINE bool all(const flt& in) { return in.i[0] != 0; }

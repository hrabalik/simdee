...
alignas(T) const T::array_e bufA = { {
    -0.27787193f, +0.70154146f, -2.05181630f, +2.22944568f,
    -0.82358653f, -1.57705702f, +0.50797465f, -0.59003456f,
} };
alignas(T) const T::array_e bufB = { {
    -0.23645458f, +2.02369089f, -2.25835397f, +2.22944568f,
    +0.33756370f, -0.87587426f, -1.66416447f, -0.59003456f,
} };
alignas(T) const T::array_e bufC = { {
    +0.42862268f, +2.02369089f, +1.87786546f, +0.94070440f,
    +0.78734578f, -0.87587426f, +0.31994913f, -0.55829428f,
} };
alignas(T) const T::array_e bufN = { {
    std::numeric_limits<T::e_t>::infinity(),
    -std::numeric_limits<T::e_t>::infinity(),
    std::numeric_limits<T::e_t>::quiet_NaN(),
    std::numeric_limits<T::e_t>::max(),
    std::numeric_limits<T::e_t>::lowest(),
    std::numeric_limits<T::e_t>::min(),
    std::numeric_limits<T::e_t>::epsilon(),
    std::numeric_limits<T::e_t>::denorm_min(),
} };
alignas(T) const T::array_e bufZ = { { 0, 0, 0, 0, 0, 0, 0, 0 } };


TEST_CASE("AVX basic guarantees", "[simd_t][x86][avx]") {
    REQUIRE(T::W == 8);
    REQUIRE((std::is_same<T::mm_t, __m256>::value));
    REQUIRE((std::is_same<T::e_t, float>::value));
    REQUIRE((std::is_same<T::array_e, std::array<T::e_t, T::W>>::value));
    REQUIRE(sizeof(T) == sizeof(T::mm_t));
    REQUIRE(alignof(T) == alignof(T::mm_t));
    REQUIRE(sizeof(T::e_t) * T::W == sizeof(T));
}

TEST_CASE("AVX explicit construction", "[simd_t][x86][avx]") {
    alignas(T)T::array_e r = bufZ;
...
TEST_CASE("AVX arithmetic", "[simd_t][x86][avx]") {
    alignas(T)T::array_e r;
    alignas(T)T::array_e e;
    ...
    SECTION("unary plus") {
        std::transform(begin(bufA), end(bufA), begin(e), [](T::e_t a) { return +a; });
        tor(+a);
        REQUIRE(r == e);
    }
    SECTION("unary minus") {
        std::transform(begin(bufA), end(bufA), begin(e), std::negate<T::e_t>());
        tor(-a);
        REQUIRE(r == e);
    }
    SECTION("plus") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), std::plus<T::e_t>());
        tor(a + b);
        REQUIRE(r == e);
    }
    SECTION("minus") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), std::minus<T::e_t>());
        tor(a - b);
        REQUIRE(r == e);
    }
    SECTION("multiplies") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), std::multiplies<T::e_t>());
        tor(a * b);
        REQUIRE(r == e);
    }
    SECTION("divides") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), std::divides<T::e_t>());
        tor(a / b);
        REQUIRE(r == e);
    }
}
...
TEST_CASE("AVX comparison", "[simd_t][x86][avx]") {
    ...
    alignas(T)std::array<T::u_t, T::W> r;
    alignas(T)std::array<T::u_t, T::W> e;
    T a = simd::aligned(bufA.data());
    T b = simd::aligned(bufB.data());
    auto tor = [&r](const T& t) {
        simd::aligned(r.data()) = t;
    ...
    SECTION("equal to") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), [&if_](T::e_t a, T::e_t b) {
            return if_(a == b);
        });
        tor(a == b);
        REQUIRE(r == e);
...
TEST_CASE("AVX bitwise", "[simd_t][x86][avx]") {
    ...
    SECTION("bit and") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), [](T::e_t a, T::e_t b) {
    ...
    SECTION("bit not") {
        std::transform(begin(bufA), end(bufA), begin(e), [](T::e_t a) {
            return ~simd::tou(a);
        });
        tor(~a);
        REQUIRE(r == e);
    ...
    SECTION("complex expr") {
        std::transform(begin(bufA), end(bufA), begin(bufB), begin(e), [](T::e_t af, T::e_t bf) {
            auto a = simd::tou(af);
            auto b = simd::tou(bf);
            return ~((~a & ~b) | (~a ^ ~b));
        });
